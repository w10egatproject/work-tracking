...
export default function TaskTable({ tasks, onDeleteTask }: Props) {
  ...
  if (tasks.length === 0) {
    return (
      <div className="py-16 text-center bg-white rounded-2xl border border-slate-200/80 shadow-xs">
        <div className="w-12 h-12 rounded-2xl bg-sky-50 text-[#005B9A] flex items-center justify-center mx-auto mb-3 text-lg border border-sky-100">
          🔍
        </div>
  ...
  return (
    <div className="w-full">
      {/* 📋 EGAT Modern Operations Master Table */}
      <div className="bg-white rounded-2xl border border-slate-200/80 shadow-[0_1px_3px_rgba(15,23,42,0.03),0_4px_12px_rgba(15,23,42,0.02)] overflow-hidden">
        {/* Table Subheader Bar */}
        <div className="bg-slate-50 border-b border-slate-200/80 px-4 sm:px-5 py-3 flex flex-wrap items-center justify-between gap-3">
                ...
                return (
                  ...
                  >
                    ...
                    <td className="py-3 px-4 text-center font-bold">
                      <span className="inline-block bg-slate-100 text-slate-700 border border-slate-200/80 px-2 py-0.5 rounded-md font-mono text-[11px] group-hover:bg-sky-100 group-hover:text-[#005B9A] group-hover:border-sky-200 transition-colors">
                    ...
                    <td className="py-3 px-3.5 text-center font-mono font-bold">
                      <span className="inline-block bg-sky-50 border border-sky-200 text-[#005B9A] px-2 py-0.5 rounded-md text-[11px]">
                        {task.wo || "-"}
                      </span>
                    </td>
                    ...
                    <td className="py-3 px-4 text-slate-600">
                      {task.equip ? (
                        <div className="flex items-center gap-1.5 bg-slate-50 border border-slate-200/80 px-2.5 py-1 rounded-md max-w-[220px]">
                    ...
                    <td className="py-3 px-3 text-center">
                      {task.link ? (
                        <a
                          ...
                          className="inline-flex items-center gap-1 text-[#005B9A] hover:text-[#004A7D] bg-sky-50 hover:bg-sky-100 border border-sky-200/80 px-2 py-0.5 rounded-md text-[11px] font-semibold transition-all shadow-2xs"
                    ...
                    <td className="py-3 px-4 text-right whitespace-nowrap">
                      <div className="flex items-center justify-end gap-1.5 whitespace-nowrap">
                        <a
                          href={`/task/${task.id}`}
                          className="inline-flex items-center justify-center gap-1 px-3 py-1.5 bg-[#005B9A] hover:bg-[#004A7D] text-white rounded-xl text-xs font-semibold transition-all duration-150 shadow-2xs group/btn active:scale-95 whitespace-nowrap shrink-0"
                        ...
                        <button
                          ...
                          className="inline-flex items-center justify-center p-1.5 text-slate-400 hover:text-rose-600 hover:bg-rose-50 border border-transparent hover:border-rose-200 rounded-xl transition-all duration-150 cursor-pointer active:scale-95 shrink-0"
        ...
        <div className="bg-slate-50 border-t border-slate-200/80 px-4 sm:px-5 py-2.5 flex flex-wrap items-center justify-between gap-3 text-xs text-slate-500">
      ...
      {/* Toast Notification */}
      {toastMessage && (
        <div className="fixed bottom-16 md:bottom-6 right-6 z-50 bg-[#0F172A] text-white px-4 py-3 rounded-2xl shadow-xl flex items-center gap-2.5 border border-slate-700 animate-in slide-in-from-bottom-5 duration-200">
